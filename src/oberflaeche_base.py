...
fehlgeschlagen.\n{ex}",
    ...
    def _get_Label_content(self, key: str) -> str:
        ...
        """
        content = (
            self.middleware.ini_file.get_active_company_content()
            if self.middleware and hasattr(self.middleware.ini_file, "get_active_company_content")
            else self.middleware.ini_file.content
        )
    ...
    def makeform(self, type: str, offset: int = 0) -> dict:
        ...
        if self.middleware.ini_file:
            content = (
                self.middleware.ini_file.get_active_company_content()
                if hasattr(self.middleware.ini_file, "get_active_company_content")
                else self.middleware.ini_file.read_ini_file()
            )
    ...
    def load_values_into_entries(self, content: dict) -> None:
        ...
        for key, field in self.ents.items():
            ...
            elif key in self.menuvars:
                self.menuvars[key].set("1" if value in ["Ja", "1", True] else "0")
    ...
    def _check_content_of_stammdaten(self, content: dict) -> bool:
        ...
        if self.middleware and hasattr(
            self.middleware.ini_file, "get_active_company_content"
        ):
