...
class ExcelContent:
    ...
    def __init__(self, filename: str, directory: str):
        ...
        self.path: str = (
            os.path.join(self.dir, self.fn) if "/" not in filename else filename
        )
    ...
    def read_sheet(self, sheet_name) -> pd.DataFrame:
        ...
        self.daten = pd.read_excel(io=self.xlsx, sheet_name=sheet_name, header=None)
        self.daten.columns = list(string.ascii_uppercase)[0 : len(self.daten.columns)]
        self.daten.index = range(1, len(self.daten.index) + 1)
        # print(self.daten)
        return self.daten

    def mapReducePositions(
        self, inp: pd.DataFrame, columns: list, start_row: str = "1"
    ) -> pd.DataFrame:
 ...
 nicht finden."
    ...
    def _search_cell_in_column(self, column_name: str, search_value: str) -> list:
        """
        Search in row with column_name for search_value,
            return array with values right of the search_value
        """
        SEARCH_ERR = ValueError(self._get_search_err(search_value, column_name))
    ...
    def _get_value_of_row_in_next_column(
        self, arr: list, col: str, error_msg: str, raiseIfNotFound: bool = True
    ) -> any:
    ...
    def search_cell_right_of(
        self, column_name: str, search_value: str, raiseIfNotFound: bool = True
    ) -> any:
        """
        Search in row with column_name for search_value,
        ...
        return self._get_value_of_row_in_next_column(
            arr,
            column_name,
    ...
    def _search_anschrift(
        self, spalte: str, zeile: str, search: str, customer: Kunde
    ) -> None:
        """
        ...
        fromIdx = (
            self.daten.index.to_list().index(int(zeile))
            if zeile
            else self.daten.index[an == search].tolist()[0]
        )
 ...
 die Spalte '{column_name}' nicht finden."
    ...
    def _get_line_with_search_value_in_column(
        self, column_name: str, search_value: str
    ) -> pd.DataFrame:
    ...
    def _split_dataframe_by_search_value(
        self, column_name: str, search_value: str
    ) -> pd.DataFrame:
        """
        ...
        if self.daten is None:
            return None
        line = self._get_line_with_search_value_in_column(column_name, search_value)
        SEARCH_ERR, COL_ERR = self._get_err_objects(column_name, search_value)
        # print(line)
        try:
        ...
        tmpdf = self.daten.iloc[start_index : len(self.daten), :]  # noqa: E203
    ...
    def get_address_of_customer(
        self,
        spalte: str = ANSCHRIFT_DEFAULT_SPALTE,
        anschrift: str = "An:",
        customer: Kunde = None,
        zeile: str = None,
    ):
    ...
    def get_invoice_number(
        self,
        spalte: str = RGNR_DEFAULT_SPALTE,
        ...
        theDict = {
            rg_nr: (
                self._get_content_at(spalte, zeile)
                if zeile
                else self.search_cell_right_of(spalte, rg_nr)
            )
        }
    ...
    def get_invoice_date(
        self,
        spalte: str = RGDATUM_REFAULT_SPALTE,
    ...
    def get_invoice_positions(
        self, spalte: str = "A", search: str = "Pos.", mgmnt: Steuerung = None
    ) -> dict:
        """
        ...
        if mgmnt and mgmnt.positionen_zeile:
            ...
            return self.mapReducePositions(self.daten, columns, mgmnt.positionen_zeile)
    ...
    def _round(self, value, prec: int) -> decimal.Decimal:
        ...
        precStr = "1." + "0" * prec
        rounded = decimal.Decimal(value + 0.0000000001).quantize(
            decimal.Decimal(precStr), rounding=decimal.ROUND_HALF_UP
        )
        # print("_round:", value, ' = ', rounded)
        return rounded

    def _get_netto_sum(self, spalte: str, search: str, management: Steuerung) -> any:
        if management is not None and management.nettosumme_zeile is not None:
            return self._get_content_at(
                management.nettosumme_spalte, management.nettosumme_zeile
            )
        else:
            return self.search_cell_right_of(spalte, search)

    def _get_ust_sum(self, spalte: str, search: str, management: Steuerung) -> any:
        if management is not None and management.mwstsumme_zeile is not None:
            return self._get_content_at(
                management.mwstsumme_spalte, management.mwstsumme_zeile
            )
        else:
            return self.search_cell_right_of(spalte, search)

    def _get_brutto_sum(self, spalte: str, search: str, management: Steuerung) -> any:
        if management is not None and management.bruttosumme_zeile is not None:
            return self._get_content_at(
                management.bruttosumme_spalte, management.bruttosumme_zeile
            )
        else:
            return self.search_cell_right_of(spalte, search)

    def get_invoice_sums(
        self,
        spalte: str = "F",
        sum: str = "Summe",
        ...
        return [
            ("Summe netto:", self._round(netto, 2)),
