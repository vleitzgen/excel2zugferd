...
class InvoiceCollection(Invoice):
    ...
    def __init__(self, daten: ExcelContent = None, stammdaten: dict = None) -> None:
        super().__init__()
    ...
    def set_daten(self, daten: ExcelContent = None) -> None:
        if daten is not None:
            self.customer = Kunde()
            spalte = (
                self.management.anschrift_spalte
                if self.management.anschrift_spalte
                else ANSCHRIFT_DEFAULT_SPALTE
            )
            daten.get_address_of_customer(
                spalte=spalte,
                zeile=self.management.anschrift_zeile,
            ...
            spalte = (
                self.management.rechnung_spalte
                if self.management.rechnung_spalte
                else RGNR_DEFAULT_SPALTE
            )
            self.invoicenr = daten.get_invoice_number(
                spalte=spalte, zeile=self.management.rechnung_zeile
            )
            spalte = (
                self.management.datum_spalte
                if self.management.datum_spalte
                else RGDATUM_REFAULT_SPALTE
            )
            self.invoicedate = daten.get_invoice_date(
                spalte=spalte, zeile=self.management.datum_zeile
            )
